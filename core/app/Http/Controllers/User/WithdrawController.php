...
class WithdrawController extends Controller
{
    ...
    public function withdrawMoney()
    {
        ...
        return view('Template::user.withdraw.methods', compact('pageTitle','withdrawMethod','wallets'));
    }
    ...
    public function withdrawStore(Request $request)
    {
        ...
        if ($request->amount > $user->walletBalance($request->wallet_type)) {
            $notify[] = ['error', 'Insufficient balance in ' . User::walletLabel($request->wallet_type) . ' for withdrawal'];
            return back()->withNotify($notify)->withInput($request->all());
        }
    ...
    public function withdrawSubmit(Request $request)
    {
        ...
        $walletColumn = User::walletColumn($withdraw->wallet_type);

        if ($withdraw->amount > $user->$walletColumn) {
            $notify[] = ['error', 'Your request amount is larger than your current ' . User::walletLabel($withdraw->wallet_type) . ' balance.'];
            return back()->withNotify($notify)->withInput($request->all());
        }
        ...
        $withdraw->save();
        $user->$walletColumn -= $withdraw->amount;
        $user->save();
        ...
        $transaction->wallet_type = $withdraw->wallet_type;
        $transaction->save();
