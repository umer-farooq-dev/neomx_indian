...
class ManageUsersController extends Controller
{
    ...
    public function addSubBalance(Request $request, $id)
    {
        $request->validate([
            ...
            'wallet_type' => 'required|in:main,referral,reward',
        ...
        $trx          = getTrx();
        $walletColumn = User::walletColumn($request->wallet_type);
        $walletLabel  = User::walletLabel($request->wallet_type);
        ...
        if ($request->act == 'add') {
            ...
            $notify[] = ['success', $walletLabel . ' credited successfully'];
        } else {
            if ($amount > $user->$walletColumn) {
                $notify[] = ['error', $user->username . ' doesn\'t have sufficient ' . $walletLabel . '.'];
                return back()->withNotify($notify);
            }
            ...
            $notify[]       = ['success', $walletLabel . ' debited successfully'];
        }
        ...
        $transaction->post_balance = $user->$walletColumn;
        $transaction->charge       = 0;
        $transaction->trx          = $trx;
        $transaction->details      = $request->remark . ' (' . $walletLabel . ')';
        $transaction->save();
        ...
        notify($user, $notifyTemplate, [
            ...
            'post_balance' => showAmount($user->$walletColumn, currencyFormat: false)
        ]);
    ...
    public function grantSpins(Request $request, $id)
    {
        $request->validate([
        ...
        $user = User::findOrFail($id);
        \App\Lib\SpinService::grantSpins($user, $request->count);

        $notify[] = ['success', $request->count . ' spin(s) granted to ' . $user->username];
        return back()->withNotify($notify);
    }
    ...
    public function referrals($id)
    {
        ...
        $referrals    = User::whereIn('id', $levelUserIds)->searchable(['username', 'email', 'mobile'])->with('deposits')->paginate(getPaginate());
        $levelCounts  = $tree->groupBy('lvl')->map->count();

        return view('admin.users.referrals', compact(
            'pageTitle', 'referrals', 'user', 'totalReferrals', 'activeReferrals',
