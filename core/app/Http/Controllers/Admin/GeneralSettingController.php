...
class GeneralSettingController extends Controller
{
    ...
    protected function findOrNewNotice()
    {
        $notice = Frontend::where('data_keys', 'notice.data')->first();
        if (!$notice) {
            $notice = new Frontend();
            $notice->data_keys = 'notice.data';
        }
        return $notice;
    }
    ...
    {
        $pageTitle = 'Site Notice';
        $notice    = $this->findOrNewNotice();
        if (!$notice->exists) {
            $notice->data_values = ['status' => Status::DISABLE, 'text' => ''];
            $notice->save();
        }
        return view('admin.setting.notice', compact('pageTitle', 'notice'));
    }
    ...
    public function noticeSubmit(Request $request)
    {
        ...
        $notice->save();
        $notify[] = ['success', 'Site notice updated successfully'];
        return back()->withNotify($notify);
    }
